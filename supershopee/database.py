...
def init_db():
    ...
    if curr.execute("SELECT COUNT(*) FROM users").fetchone()[0] == 0:
        users = [
            ...
            ('cashier', generate_password_hash('cashier123'), 'Customer'),
    ...
    if curr.execute("SELECT COUNT(*) FROM products").fetchone()[0] == 0:
        products = [
            ...
            ("Basmati Rice 1kg", "Pantry", 120.00, 150, get_placeholder("Basmati Rice\n1kg"),
             "Premium long-grain Basmati rice.", 1),
            ("Whole Grain Pasta", "Pantry", 110.00, 200, get_placeholder("Whole Grain\nPasta"),
             "Nutritious whole grain pasta.", 1),
            ...
            ("Green Tea Leaves", "Beverages", 180.00, 70, get_placeholder("Green Tea\nLeaves"),
             "Antioxidant-rich loose green tea.", 1)
        ]
        curr.executemany(
...
if __name__ == "__main__":
    if os.path.exists(DB_PATH):
        os.remove(DB_PATH)
    init_db()
    print("✅ Database initialized with admin, cashier, and test customer accounts!")
    print("   Admin: admin / admin123")
    print("   Cashier: cashier / cashier123")
    print("   Shopper: shopper / shopper123")